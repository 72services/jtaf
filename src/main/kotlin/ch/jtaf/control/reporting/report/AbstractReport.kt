...
abstract class AbstractReport protected constructor(protected val locale: Locale) {
    ...
    protected fun addCell(table: PdfPTable, text: String) {
        val cell = PdfPCell(
                Phrase(text, FontFactory.getFont(FontFactory.HELVETICA, DEFAULT_FONT_SIZE)))
        cell.border = 0
        table.addCell(cell)
    }
    ...
    protected fun addCell(table: PdfPTable, text: String, fontSize: Float) {
        val cell = PdfPCell(
                Phrase(text, FontFactory.getFont(FontFactory.HELVETICA, fontSize)))
        cell.border = 0
        table.addCell(cell)
    }
    ...
    internal inner class HeaderFooter(left: String, middle: String, right: String) : PdfPageEventHelper() {
        private val header: PdfPTable

        init {
            header = PdfPTable(floatArrayOf(2.5f, 4f, 1f))
        ...
        override fun onEndPage(writer: PdfWriter?, document: Document?) {
            ...
            val cellLeft = PdfPCell(
                    Phrase(sdf.format(Date()),
                            FontFactory.getFont(FontFactory.HELVETICA, DEFAULT_FONT_SIZE)))
            cellLeft.border = 0
            cellLeft.borderWidthTop = 1f
            table.addCell(cellLeft)

            val cellCenter = PdfPCell(
                    Phrase("JTAF - Track and Field | www.jtaf.ch",
                            FontFactory.getFont(FontFactory.HELVETICA, DEFAULT_FONT_SIZE)))
            cellCenter.border = 0
            ...
            val cellRight = PdfPCell(
                    Phrase("Page" + " " + document!!.pageNumber,
                            FontFactory.getFont(FontFactory.HELVETICA, DEFAULT_FONT_SIZE)))
            cellRight.border = 0
        ...
        private fun addHeaderCellAlignLeft(table: PdfPTable, text: String) {
            val cell = PdfPCell(
                    Phrase(text, FontFactory.getFont(FontFactory.HELVETICA_BOLD, HEADER_FONT)))
            cell.border = 0
        ...
        private fun addHeaderCellAlignRight(table: PdfPTable, text: String) {
            val cell = PdfPCell(
                    Phrase(text, FontFactory.getFont(FontFactory.HELVETICA_BOLD, HEADER_FONT_SMALL)))
            cell.border = 0
            cell.horizontalAlignment = PdfPCell.ALIGN_RIGHT
            cell.verticalAlignment = PdfPCell.ALIGN_BOTTOM
            table.addCell(cell)
